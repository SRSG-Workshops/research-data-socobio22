...
We can also see that the columns `genus`, `species`, `sex`, `plot_type`, ... are strings. 
Since the release of R 4.0.0 [https://stat.ethz.ch/pipermail/r-announce/2020/000653.html], 
`stringsAsFactors = FALSE` is the default.  If we want to work with these columns as 
factors, we can reload the data with `stringsAsFactors = TRUE`:
...
### Using `stringsAsFactors=FALSE` vs `stringsAsFactors=TRUE`
...
> This is great for making your code reproducible. You can do this using the package `renv`.
>
...
> or `prioritize()` from the package `man` to ensure you’re using the correct version of the function.
{: .challenge}
...
>  (Assume that `hindfoot_length` is currently in millimetres).
...
>
> ```{r, answer=TRUE, purl=FALSE, results='hide'}
> surveys %>%
>
> count(plot_type)
> ```
